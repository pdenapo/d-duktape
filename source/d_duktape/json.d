...
{
  //debug writeln("push_Json al comenzar");
  switch (json.type())
  {
  case Json.Type.int_:
    duk_push_int(ctx, json.get!int);
    break;
  case Json.Type.string:
    // No usar to!string para la conversión, que agrega comillas
    immutable(char)* s = json.get!string.toStringz();
    duk_push_string(ctx, s);
    break;
  case Json.Type.bool_:
    duk_push_boolean(ctx, json.get!bool);
    break;
  case Json.Type.float_:
    duk_push_number(ctx, json.get!double);
    break;
  ...
  default:
    writeln("Unhandled type in push_Json");
    assert(0);
  }
}
...
{
  debug writeln("duk_to_Json al comenzar");
  Json result;
  ...
  case DUK_TYPE_OBJECT:
    ...
    // the case bigInt is not supperted 
  default:
    writeln("Unhandled type in duk_to_Json ", which_type);
    assert(0);
  }
  return result;
}
